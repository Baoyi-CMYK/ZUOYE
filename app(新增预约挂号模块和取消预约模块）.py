...
from models import db, User, Department, Doctor, Schedule, Appointment
...
def get_my_appointments():
    ...
    if not user_id:
        return jsonify({'success': False, 'message': '未登录'})
    appointments = Appointment.query.filter_by(patient_id=user_id).order_by(Appointment.created_at.desc()).all()
    return jsonify({'success': True, 'data': [a.to_dict() for a in appointments]})
...
def create_appointment():
    ...
    if not all([doctor_id, appointment_date, time_period]):
    ...
    if existing:
        return jsonify({'success': False, 'message': '该时段已被预约'})
    ...
    return jsonify({'success': True, 'message': '预约成功', 'data': appointment.to_dict()})
...
def cancel_appointment(apt_id):
    ...
    if appointment.patient_id != user_id:
        return jsonify({'success': False, 'message': '只能取消自己的预约'})
    
    if appointment.status == 'cancelled':
        return jsonify({'success': False, 'message': '预约已取消'})
